...
export const addRestaurant = async (req, res) => {
  ...
  try {
    ...
    const isRestaurant =
      ...
      (await Restaurant.findOne({ contact_no }));
    if (isRestaurant) {
...
export const loginRestaurant = async (req, res) => {
  try {
    ...
    if (!restaurant) {
      return res
        .status(404)
        .json("No account exist with this email address or phone number");
    }
...
export const addItem = async (req, res) => {
  try {
    ...
    const allReadyRegister = await Restaurant.findOne({
      $and: [
        { "menu.item_name": req.body.item_name },
        { "menu.category_name": req.body.category_name },
      ],
    });
    if (allReadyRegister) {
    ...
    const result = await Restaurant.findOneAndUpdate(
      { _id: req.restaurant.restaurant_id },
      { $push: { menu: menudata } },
      { new: true }
    );
...
export const deleteItem = async (req, res) => {
  ...
  const restaurant_id = req.restaurant.restaurant_id;

  const restaurant = await Restaurant.findById({ _id: restaurant_id });

  const itemIndex = await restaurant.menu.findIndex(
    (item) => item._id.toString() === id
  );
  console.log(itemIndex);
  if (itemIndex === -1) {
    res.status.json("item doesn't exist");
  }

  await restaurant.menu.splice(itemIndex, 1);
...
export const updateItem = async (req, res) => {
  try {
    const { id } = req.params;
    const { category_name, item_name, item_picture, price } = req.body;

    const restaurant_id = req.restaurant.restaurant_id;

    const restaurant = await Restaurant.findById({ _id: restaurant_id });

    for (let element of restaurant.menu) {
      if (element._id.toString() === id) {
        element.category_name = category_name;
        element.item_name = item_name;
        element.item_picture = item_picture;
        element.price = price;
      }
    }
